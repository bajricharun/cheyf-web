...
class RentlApiService {
  ...
  private async fetchApi<T>(
    ...
    try {
      ...
      if (!response.ok) {
        ...
        console.warn(
          `Rentl API warning (${response.status}) for ${endpoint}: ${msg}`,
        );
  ...
  private async mockApiCall<T>(endpoint: string): Promise<RentlApiResponse<T>> {
    ...
    if (endpoint.startsWith("/properties")) {
      const properties: Property[] = Object.entries(propertyData).map(
        ([id, data]) => ({
          id,
          name: data.name,
  ...
  async getProperties(): Promise<RentlApiResponse<Property[]>> {
    ...
    const properties: Property[] = items.map((prop: any) => {
      ...
      const localData =
        propertyData[prop.id] || propertyData[parseInt(prop.id)];
      return {
        ...prop,
        id: prop.id.toString(),
  ...
  async getProperty(id: string): Promise<RentlApiResponse<Property>> {
    ...
    if (prop) {
      const localData =
        ...
        propertyData[parseInt(prop.id)] ||
        propertyData[id as any];
      const mergedProp = {
        ...prop,
        id: prop.id.toString(),
  ...
  async checkBulkAvailability(
    ...
    const res = await this.fetchApi<any[]>(
      `/availability?propertiesIds=${idsQuery}&dateFrom=${dateFrom}&dateTo=${dateTo}`,
    );
    ...
    if (res.status === 200 && Array.isArray(res.data)) {
      const processed = res.data.map((prop: any) => {
        ...
        if (prop.unitTypes && prop.unitTypes.length > 0) {
          ...
          const blocked = (unit.availability || []).find(
            (day: any) => day.value === 0,
          );
          isAvailable = !blocked;
        ...
        return {
          ...
          pricePerNight:
            totalPrice /
            (prop.unitTypes?.[0]?.rates?.[0]?.DailyValues?.length || 1),
        };
      });
  ...
  ): Promise<RentlApiResponse<Booking>> {
