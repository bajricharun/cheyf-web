...
export async function GET(request: Request) {
  ...
  try {
    ...
    if (Array.isArray(data) && data.length > 0) {
      const propertyAvail = data.find(
        (p: any) => p.id.toString() === propertyId.toString(),
      );
      if (
        propertyAvail &&
        propertyAvail.unitTypes &&
        propertyAvail.unitTypes.length > 0
      ) {
        // Find requested unit type, or fallback to first
        const unit = unitTypeId
          ? propertyAvail.unitTypes.find(
              (u: any) => u.id.toString() === unitTypeId.toString(),
            )
          : propertyAvail.unitTypes[0];

        if (!unit) {
          return NextResponse.json(
            { error: "Unit type not found", available: false },
            { status: 404 },
          );
        }
    ...
    return NextResponse.json(
      { error: "Internal Server Error", blockedDates: [] },
      { status: 500 },
    );
  }
}
