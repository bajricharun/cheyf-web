  ...
  try {
    ...
    if (!targetUnitTypeId) {
      const unitsRes = await fetch(
        `${RENTL_API_BASE_URL}/properties/${propertyId}/unit-types`,
        {
    ...
    const pageRequests = [1, 2, 3, 4].map((page) =>
      fetch(
        `${RENTL_API_BASE_URL}/unit-types/${targetUnitTypeId}/availability?dateFrom=${dateFrom}&dateTo=${dateTo}&page=${page}&perPage=100`,
        {
          headers: {
