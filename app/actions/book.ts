...
export async function submitReservation(prevState: any, formData: FormData) {
  try {
    ...
    let unitTypeId = formUnitTypeId ? parseInt(formUnitTypeId, 10) : parseInt(propertyId, 10);
    
    const availRes = await fetch(`${RENTL_API_BASE_URL}/availability?propertiesIds=${propertyId}&dateFrom=${dateFrom}&dateTo=${dateTo}`, {
      headers: {
    ...
    if (availRes.ok) {
      ...
      if (Array.isArray(availData) && availData.length > 0) {
        const propertyData = availData.find((p: any) => p.id.toString() === propertyId.toString());
        if (propertyData && propertyData.unitTypes && propertyData.unitTypes.length > 0) {
    ...
    if (!res.ok) {
      ...
      const errorMessage = err?.errors?.global || err?.message || 'Failed to create reservation.';
