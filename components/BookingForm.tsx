...
import { format, addDays, isBefore, startOfDay, isWithinInterval } from 'date-fns';
import { Button } from '@/components/ui/Button';
import { Input } from '@/components/ui/Input';
import { Card, CardContent } from '@/components/ui/Card';
import { Calendar as CalendarIcon, Users, Mail, User, Info, Loader2, CreditCard } from 'lucide-react';
import { Calendar } from '@/components/ui/calendar';
import { Popover, PopoverContent, PopoverTrigger } from '@/components/ui/popover';
import { cn } from '@/lib/utils';
import { DateRange } from 'react-day-picker';

export function BookingForm({ propertyId, propertyName, unitTypes }: { propertyId: string | number; propertyName: string; unitTypes?: any[] }) {
  const defaultUnitId = unitTypes && unitTypes.length > 0 ? String(unitTypes[0].id) : '';
  ...
  useEffect(() => {
    async function loadBlockedDates() {
      ...
      try {
        ...
        const res = await fetch(`/api/rentl/reservations?propertyId=${propertyId}&unitTypeId=${selectedUnitId}&dateFrom=${format(today, 'yyyy-MM-dd')}&dateTo=${format(nextYear, 'yyyy-MM-dd')}`);
        const data = await res.json();

        if (Array.isArray(data)) {
          setBlockedDates(data.map((d: any) => ({
  ...
  useEffect(() => {
    async function checkAvailability() {
      if (date?.from && date?.to) {
        ...
        try {
          const res = await fetch(`/api/rentl/availability?propertyId=${propertyId}&unitTypeId=${selectedUnitId}&dateFrom=${format(date.from, 'yyyy-MM-dd')}&dateTo=${format(date.to, 'yyyy-MM-dd')}`);
          const data = await res.json();
  ...
  const isDateDisabled = (day: Date) => {
    ...
    return blockedDates.some(range => isWithinInterval(day, { start: startOfDay(range.from), end: startOfDay(range.to) }));
  };

  // Determine if booking is allowed
  const isMinStayViolated = availability?.error || (availability?.minStay && date?.from && date?.to && ((date.to.getTime() - date.from.getTime()) / (1000 * 3600 * 24)) < availability.minStay);
  const validationError = availability?.error || (isMinStayViolated ? `Minimum stay for these dates is ${availability?.minStay} nights.` : null);

  const amountToPay = availability?.totalPrice 
    ? (paymentOption === 'deposit' ? availability.totalPrice * 0.4 : availability.totalPrice) 
    : 0;
  ...
  const handlePaymentSubmit = async (e: FormEvent<HTMLFormElement>) => {
    ...
    try {
      ...
      // Generate the invisible form and submit it
      ...
      const fields = {
        ...
        // Optional custom params for our callback to read
        ...
        <form onSubmit={handlePaymentSubmit} className="space-y-4">
          ...
          {unitTypes && unitTypes.length > 1 && (
            <div className="space-y-2">
              <label htmlFor="unitTypeId" className="text-sm font-medium flex items-center gap-2">
                  ...
                  <div className="space-y-4">
                    <div className="flex justify-between text-sm">
                      <span className="text-muted-foreground">
                         &euro;{availability.pricePerNight?.toFixed(2)} x {Math.ceil((date.to.getTime() - date.from.getTime()) / (1000 * 3600 * 24))} nights
                      </span>
                    ...
                    <div className="pt-4 border-t border-border/50">
                      ...
                      <div className="grid grid-cols-2 gap-3">
                        <label className={cn(
                          ...
                          paymentOption === 'deposit' ? "border-accent bg-accent/10" : "border-border hover:border-accent/50"
                        )}>
                          <input type="radio" name="paymentOption" value="deposit" className="sr-only" onChange={() => setPaymentOption('deposit')} checked={paymentOption === 'deposit'} />
                          <span className="text-xs font-medium text-muted-foreground mb-1">Pay 40% Deposit</span>
                          <span className="font-bold text-accent">&euro;{(availability.totalPrice * 0.4).toFixed(2)}</span>
                        </label>
                        
                        <label className={cn(
                          "cursor-pointer rounded-lg border p-3 flex flex-col items-center justify-center text-center transition-all",
                          paymentOption === 'full' ? "border-accent bg-accent/10" : "border-border hover:border-accent/50"
                        )}>
                          <input type="radio" name="paymentOption" value="full" className="sr-only" onChange={() => setPaymentOption('full')} checked={paymentOption === 'full'} />
                          <span className="text-xs font-medium text-muted-foreground mb-1">Pay Full Amount</span>
                          <span className="font-bold text-accent">&euro;{availability.totalPrice.toFixed(2)}</span>
                        </label>
                      </div>
                    </div>
                    ...
                    <div className="pt-2 mt-2 border-t border-border/50 flex justify-between font-bold text-lg">
                      ...
                      <span className="text-accent">&euro;{amountToPay.toFixed(2)}</span>
                    </div>
                  </div>
               ) : null}
          ...
          <div className="space-y-2">
            ...
            <Input id="guests" name="guests" type="number" min="1" max={10} required placeholder="2" />
          </div>
          ...
          <div className="space-y-2">
            <label htmlFor="note" className="text-sm font-medium flex items-center gap-2">
              <Info className="w-4 h-4 text-muted-foreground" /> Special Requests / Note
            </label>
            <textarea 
              id="note" 
              name="note" 
          ...
          <Button 
            ...
            variant="accent" 
            disabled={!date?.from || !date?.to || !!validationError || isLoadingPrice || isProcessingPayment}
          >
            {isProcessingPayment ? <><Loader2 className="w-4 h-4 animate-spin" /> Redirecting to Secure Payment...</> : <><CreditCard className="w-4 h-4" /> Go to Secure Checkout</>}
          </Button>
        </form>
      </CardContent>
    </Card>
